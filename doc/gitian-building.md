...
*Setup instructions for a gitian build of Dankcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Dankcoin
...
- [Building Dankcoin](#building-dankcoin)
...
Clone the git repositories for dankcoin and gitian and then checkout the dankcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the dankcoin repository
...
To build Dankcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the dankcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dankcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/dankcoin/dankcoin
...
For example:
```bash
URL=https://github.com/user/dankcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit dankcoin=${COMMIT} --url dankcoin=${URL} ../dankcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dankcoin=${COMMIT} --url dankcoin=${URL} ../dankcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dankcoin=${COMMIT} --url dankcoin=${URL} ../dankcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dankcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dankcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/dankcoin-build.assert
```
...
[dankcoin/gitian.sigs](https://github.com/dankcoin/gitian.sigs.pepe/) repository, or if that's not possible create a pull
request. 
