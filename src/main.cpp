...
#if defined(NDEBUG)
# error "Dankcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Dankcoin Signed Message:\n";
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees)
{
    ...
    // Subsidy is cut in half every 450,500 blocks.
    nSubsidy >>= halvings;
...
void ThreadScriptCheck() {
    RenameThread("dankcoin-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Dankcoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Dankcoin: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
